...
Site.ScrollManager = function() {
    ...
    this.windowDidScroll = function(){
        ...
        var windowMid = windowTop + Site.wh;
        var windowBot = windowTop + Site.wh;

        var deltas = [];
        for (i=0; i<this.sections.length; i++){
            var section = this.sections[i];
            var itemMid = section.rect.top + section.rect.height * 1;
            var delta = Math.abs(itemMid - windowMid);
            delta = Math.max(delta - Site.wh * 0.15, 0.0);
            section.div.style.opacity = Math.pow(1.0 - (delta / (Site.wh * 0.8)), 4.0);
